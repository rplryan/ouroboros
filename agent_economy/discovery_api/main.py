...
# This speaks proper MCP JSON-RPC 2.0 protocol
# POST /smithery  — MCP endpoint for Smithery and other MCP clients
# ---------------------------------------------------------------------------
...
async def _handle_mcp_tool_call(tool_name: str, arguments: dict, request: Request) -> dict:
    ...
    if tool_name == "x402_browse":
        ...
        for s in services:
            ...
            by_category.setdefault(cat, []).append(_enrich_with_quality(s))
        return {
            "type": "text",
            "text": json.dumps({
                "total": len(services),
        ...
        return {
            ...
            "text": json.dumps({
                "service_id": service_id,
                "name": entry.get("name"),
                "status": _compute_health_status(stats, last or {}),
                "uptime_pct": stats.get("uptime_pct"),
                "avg_latency_ms": stats.get("avg_latency_ms"),
                "last_checked": last.get("checked_at") if last else None,
            }, indent=2),
        ...
        if not payment_header:
            challenge = _payment_required_body(host, resource_path, QUERY_PRICE_UNITS, "x402 Service Discovery Query")
            return {
                "type": "text",
                "text": json.dumps({
        ...
        if not is_valid:
            return {"type": "text", "text": json.dumps({"error": "Payment verification failed", "payment_required": True}), "isError": True}
...
async def smithery_mcp(request: Request) -> JSONResponse:
    ...
    if method == "initialize":
        ...
        try:
            content = await _handle_mcp_tool_call(tool_name, arguments, request)
            return JSONResponse({
                "jsonrpc": "2.0",
                "id": req_id,
                "result": {
...
async def mcp_server_card() -> JSONResponse:
    ...
    return JSONResponse({
        ...
        "description": "Discovers x402-payable APIs at runtime. Enables autonomous agents to find, evaluate, and pay for services via USDC micropayments on Base — no API keys or subscriptions required.",
